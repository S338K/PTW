...
(function () {
    try {
        ...
        function getCurrentTheme() {
            ...
            return html.classList.contains('dark') ? 'dark' : (html.getAttribute('data-theme') || 'light');
        }
        ...
        function toggleTheme() {
            ...
            applyTheme(next);
            persistTheme(next);
            try { window.dispatchEvent(new CustomEvent('theme:changed', { detail: { theme: next } })); } catch (_) { }
        }
        ...
        // wire
        document.querySelectorAll('[data-theme-toggle]').forEach((el) => {
            el.addEventListener('click', (e) => { e.preventDefault(); toggleTheme(); updateToggleIcons(); });
            el.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleTheme(); updateToggleIcons(); } });
        });
