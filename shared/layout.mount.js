...
(function () {
    ...
    function dedupeAndAppendHeadNodes(nodes, layoutBaseUrl) {
        ...
        nodes.forEach((node) => {
            ...
            if (clone.tagName === 'SCRIPT' && clone.src) {
                ...
                const exists = Array.from(document.scripts).some(s => s.src === clone.src);
                if (exists) return;
    ...
    ready(async function () {
        ...
        const layoutSrc = (currentScript && currentScript.dataset && currentScript.dataset.layout) || '../shared/layout.html';

        // Capture the page's original content to place inside the layout slot later
        const originalBodyChildren = Array.from(document.body.childNodes).filter(n => {
        ...
        const headNodes = Array.from(layoutDoc.head.children).filter(n => n.tagName === 'LINK' || n.tagName === 'SCRIPT');
        ...
        Array.from(document.querySelectorAll('script[src]')).forEach(scr => {
            ...
            if (srcAbs === expectedLayoutJs) {
                try { scr.remove(); } catch (e) { /* ignore */ }
            }
        });

        // Avoid double-loading if the host page already included the same script explicitly
        const alreadyLoaded = Array.from(document.scripts).some(s => s.src === expectedLayoutJs);
        if (!alreadyLoaded) {
        ...
        try {
            const bodyScripts = Array.from(layoutDoc.body.querySelectorAll('script[src]'));
            bodyScripts.forEach((scr) => {
                ...
                Array.from(document.querySelectorAll('script[src]')).forEach(s => {
                    ...
                    if (sAbs === srcAbs) { try { s.remove(); } catch (e) { /* ignore */ } }
                });
        ...
        // Signal completion
        try { window.dispatchEvent(new CustomEvent('layout:mounted')); } catch (e) { }
    });
})();
