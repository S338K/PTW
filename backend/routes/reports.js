  ...
  try {
    ...
    if (format === 'csv') {
      ...
      try {
        ...
        console.error(
          'json2csv not available. Please run `npm install` in the backend directory.',
          e && e.message ? e.message : e
        );
        return res
          .status(500)
          .json({ message: 'CSV export not available on this server (missing dependency)' });
      }

      const parser = new Parser({
        fields: fieldDefs.map((f) => ({ label: f.label, value: f.key })),
      });
    ...
    if (format === 'xlsx') {
      ...
      records.forEach((r) => {
        const values = fieldDefs.map((f) => {
          if (['submittedOn', 'preApprovedOn', 'approvedOn'].includes(f.key))
            return r[f.key] ? new Date(r[f.key]) : '';
          return r[f.key] || '';
        });
        sheet.addRow(values);
      });
    ...
    if (format === 'pdf') {
      ...
      const groupStartX = Math.max(
        ...
        Math.floor((doc.page.width - groupWidth) / 2)
      );
      ...
      doc
        ...
        .fontSize(10)
        .fillColor('gray')
        .text(`Report Period: ${period}`, metaX, headerTop + 12, {
          width: metaWidth,
          align: 'center',
        });
      doc
        ...
        .fontSize(10)
        .fillColor('gray')
        .text(`Generated: ${generated}`, metaX, headerTop + 30, {
          width: metaWidth,
          align: 'center',
        });
      ...
      doc
        .moveTo(doc.page.margins.left, headerBottom - 6)
      ...
      const renderHeader = (y) => {
        ...
        fieldDefs.forEach((f, i) => {
          ...
          doc
            .fillColor('#FFFFFF')
            .text(f.label.toUpperCase(), x + 6, textY, { width: w - 12, align: 'center' });
          x += w;
        });
        return y + h;
      };
