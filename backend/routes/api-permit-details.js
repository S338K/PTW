...
router.post('/permits/:id/action', async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(id)) return res.status(400).json({ message: 'Invalid permit ID' });
        if (!action || !['approve', 'reject'].includes(action)) return res.status(400).json({ message: 'Invalid action' });
        // simple validation for comments
        if (!comments || String(comments).trim().length < 3) return res.status(400).json({ message: 'Comments are required (min 3 characters)' });
        ...
        if (action === 'approve') {
            ...
            if (!permit.permitNumber) {
                ...
                const countToday = await Permit.countDocuments({ approvedAt: { $gte: new Date(now.setHours(0, 0, 0, 0)), $lte: new Date(now.setHours(23, 59, 59, 999)) }, status: 'Approved' });
